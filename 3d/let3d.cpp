...
// ----------------------------------------------------------------------
template <class N>
int Let3d<N>::setFromOptions(map<string,string>& optionsMap)
{
  //------------
	 ...
	 for(int k=arrBeg; k < arrEnd; k++) {
		...
		if( lclSrcNumVec[k]>ptsMax() && level<maxLevel()-1 ) {
		  ...
		  for(int a=0; a<2; a++) {
			 for(int b=0; b<2; b++) {
				for(int c=0; c<2; c++) {
				  nodeVec.push_back( N(k,-1, 2*nodeVec[k].path2Node()+Index3(a,b,c), nodeVec[k].depth()+1) ); //par, chd
				  vecIdxs.push_back( vector<int>() );
...
int Let3d<N>::trgData()
{
  ...
  for(int i=0; i < orderBoxesVec	.size(); i++) {
	 ...
	 if(curNode.child()!=-1) { //not terminal
		...
		for(vector<int>::iterator curVecIdxsIt=curVecIdxs.begin();curVecIdxsIt !=curVecIdxs.end(); curVecIdxsIt++) {
		  ...
		  /* For Harper - keep track of which terminal each point belongs to */
		  //trgPosTrmGni()[*curVecIdxsIt] = chdGNodeIdx;
		}
  ...
  for(int i=0; i<orderBoxesVec.size(); i++) {
	 int gNodeIdx = orderBoxesVec	[i];
	 //cerr << gNodeIdx << " " << lclSrcNumVec[gNodeIdx] << endl;
	 if (lclSrcNumVec[gNodeIdx] > 0){ /*  No LONGER Only Treat NON-ZERO source nodes as targets - all nodes are targets! */
  ...
  //for (int i = 0; i < trgPosTrmGni().size(); i++){ iA(terminal(trgPosTrmGni()[i])); }
  return 0;
}
...
template <class N>
int Let3d<N>::calgnext(int gNodeIdx)
{
  vector<N>&  nodeVec = this->nodeVec();
				  ...
				  if(!adj) {
					 ...
					 if(terminal(curGNodeIdx)) {
						...
						while(rest.empty()==false) {
						  int fntGNodeIdx = rest.front(); rest.pop();
...
// ----------------------------------------------------------------------
template <class N>
int Let3d<N>::dwnOrderCollect(vector<int>& orderBoxesVec	)
{
  orderBoxesVec.clear();
...
template <class N>
int Let3d<N>::upwOrderCollect(vector<int>& orderBoxesVec	)
{
  orderBoxesVec.clear();
...
// ---------------------------------------------------------------------- 
#undef __FUNCT__
#define __FUNCT__ "Let3d::revLvlOrderCollect"
template <class N>
int Let3d<N>::revLvlOrderCollect(map<int, vector<int> >& orderBoxesVec	)
{
  orderBoxesVec.clear();
  int cnt = 0;
  int levcnt = 0;
  for (int j = 2+_maxLevel; j >= 0; j--){
	 vector<int>& thisLevBoxes = orderBoxesVec[j];
	 for(int i=nodeVec().size()-1; i>=0; i--) {
		if (depth(i) == j) {
		  thisLevBoxes.push_back(i);
		  cnt++;
		}
	 }
	 levcnt += thisLevBoxes.size();
  }
  cerr << cnt << " " << levcnt << " " << orderBoxesVec.size() << " " << nodeVec().size() << endl;
  iA(cnt == levcnt);
  iA(cnt==nodeVec().size());
  return 0;
}
// ----------------------------------------------------------------------
template <class N>
int Let3d<N>::child(int gNodeIdx, const Index3& idx)
{
...
// ---------------------------------------------------------------------- 
template <class N>
Point3 Let3d<N>::center(int gNodeIdx) //center of a node
{
...
// ---------------------------------------------------------------------- 
template <class N>
double Let3d<N>::radius(int gNodeIdx) //radius of a node
{
  return radius()/double(pow2(depth(gNodeIdx)));
}
// ----------------------------------------------------------------------
template <class N>
int Let3d<N>::findgnt(int wntdepth, const Index3& wntpath)
{
...
// ----------------------------------------------------------------------
template <class N>
bool Let3d<N>::adjacent(int me, int yo)
{
