...
// ----------------------------------------------------------------------
template <class N>
int FMM3d<N>::setFromOptions(map<string,string>& opts)
{
  //-----------------------------------------------------
  ...
  case KNL_LAP_D_U: _knl_mm = Kernel3d(KNL_LAP_S_U, _knl.coefs()); break;
	 //Mod Lap kernels
  case KNL_MODHEL_S_U: _knl_mm = Kernel3d(KNL_MODHEL_S_U, _knl.coefs()); break;
  case KNL_MODHEL_D_U: _knl_mm = Kernel3d(KNL_MODHEL_S_U, _knl.coefs()); break;
	 //stokes kernels
  ...
  _matmgnt =  new MatMgnt3d(_knl_mm, _np);

  //1. _let
  _let = new Let3d<N>(prefix()+"let3d_");
  iC( _let->setFromOptions(opts) );
