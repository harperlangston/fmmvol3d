...
public:
  ...
  * Otherwise, point _data to the data input */
  ...
  F* clmdata(int i) const { return &(_data[i]); }
  /*! Return size m */
  int m () const { return _m; }

  //------------ASSIGN
 /*! Overloaded = operator.  this._data[0] = c._data[0], etc. */
  //NumVec& operator= ( const NumVec& c ) { for(int i=0; i<_m; i++) { _data[i] =c._data[i]; } return *this; }
  /*! Overloaded += operator.  this._data[0] += c._data[0], etc. */
  NumVec& operator+= ( const NumVec& c ) { for(int i=0; i<_m; i++) { _data[i] +=c._data[i]; } return *this; }
  /*! Overloaded -= operator.  this._data[0] -= c._data[0], etc. */
  NumVec& operator-= ( const NumVec& c ) { for(int i=0; i<_m; i++) { _data[i] -=c._data[i]; } return *this; }
  /*! Overloaded *= operator.  this._data[0] *= c._data[0], etc. */
  NumVec& operator*= ( const NumVec& c ) { for(int i=0; i<_m; i++) { _data[i] *=c._data[i]; } return *this; }
  NumVec& operator*= ( const F& s ) { for(int i=0; i<_m; i++) { _data[i] *=s; } return *this; }
  /*! Overloaded /= operator.  this._data[0] /= c._data[0], etc. */
  NumVec& operator/= ( const F& s ) { assert(s != 0.0); for(int i=0; i<_m; i++) { _data[i] /=s; } return *this; }
  ...
  //{ cur=max(cur,abs(_data[i])); }return cur; }
};
