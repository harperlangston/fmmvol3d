...
void func(long *ndim, double *pnt, long *Xnumfun, double *funvls)
{
  ...
  if (r > 1e-15){
	 ...
	 if (glb_kt == 111){ /* Laplacian */
		...
		double rx2 = rx*rx;  double rxry = rx*ry; double rxrz = rx*rz;
		                     double ry2 = ry*ry;  double ryrz = ry*rz;
									                     double rz2 = rz*rz;
		double x2 = x*x;     double xy = x*y;     double xz = x*z;
		                     double y2 = y*y;     double yz = y*z;
									                     double z2 = z*z;
		double x3 = x2*x;    double y3 = y2*y;    double z3 = z2*z;
	 ...
	 /*else { printf("Either you selected wrong kernel or not yet implemented\n"); assert(0);  }*/
	 else  { assert(-1); }
  }
  else {
	 int ii;
...
void testFunc(long *ndim, double *pnt, long *Xnumfun, double *funvls)
{
  double OOFP = 1.0/(4.0*M_PI);
  double x = pnt[0];
  double y = pnt[1];
  double z = pnt[2];
  /* printf("%f %f %f %f %f %f\n", glb_xtarg, glb_ytarg, glb_ztarg, x, y, z); */
  double rx = x - glb_xtarg;
  double ry = y - glb_ytarg;
  double rz = z - glb_ztarg;
  double r = sqrt(rx*rx + ry*ry + rz*rz);
  double p = 1.0 + x + y + z + x*x + y*y  + z*z + x*y + x*z + y*z + x*x*y + x*y*y + x*y*z + x*x*x + y*y*z + y*z*z + y*y*y + x*x*z + x*z*z + z*z*z;
  double v = 1.0 + x + y + z + x*x + y*y  + z*z + x*y + x*z + y*z + x*x*y + x*y*y + x*y*z + x*x*x + y*y*z + y*z*z + y*y*y + x*x*z + x*z*z + z*z*z;
  double oor = p/r;
  if (r > 1e-14){
	 funvls[0] = 1.0 * oor * OOFP;
  }
  else {
	 funvls[0] = 0.0;
  }
